...
const (
	...
	PROBE_OWNER_LABEL = "dns.kuadrant.io/probes/owner"
)
...
func RemoveUnhealthyEndpoints(ctx context.Context, clt client.Client, endpoints []*endpoint.Endpoint, dnsRecord *v1alpha1.DNSRecord) ([]*endpoint.Endpoint, error) {
	probes := &v1alpha1.DNSHealthCheckProbeList{}
	ownerLabelReq, err := labels.NewRequirement(PROBE_OWNER_LABEL, selection.Equals, []string{fmt.Sprintf("%s/%s", dnsRecord.Namespace, dnsRecord.Name)})
	if err != nil {
		return endpoints, err
	}
	ownerLabel := labels.NewSelector()
	ownerLabel.Add(*ownerLabelReq)
	clt.List(ctx, probes, &client.ListOptions{LabelSelector: ownerLabel})

	for _, probe := range probes.Items {
		//if probe is healthy or unknown, continue to next probe
		...
		for i, ep := range endpoints {
			for ti, target := range ep.Targets {
				if target == probe.Spec.Address {
					endpoints[i].Targets = append(endpoints[i].Targets[:ti], endpoints[i].Targets[:ti+1]...)
				}
			}
		}
	}
